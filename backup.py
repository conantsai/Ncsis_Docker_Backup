...
def incremental_backup(container, id, conimg, time_stamp, big, ib_path):
    ...
    # # execute full backup when diff to most
    # if (len(fs_diffadd) + len(fs_diffdelete) + len(fs_diffmodify)) > 0:
...
def backup():
    ...
    for id in result_conid:
        ...
        if os.path.exists("/usr/local/Ncsis_Docker_Backup/backup/" + id + "_" + conimg ):
            pass
        else:
            os.mkdir("/usr/local/Ncsis_Docker_Backup/backup/" + id + "_" + conimg)
            os.mkdir("/usr/local/Ncsis_Docker_Backup/backup/" + id + "_" + conimg + "/full_backup")  
            os.mkdir("/usr/local/Ncsis_Docker_Backup/backup/" + id + "_" + conimg + "/incremental_backup") 
        
        fb_path = "/usr/local/Ncsis_Docker_Backup/backup/" + id + "_" + conimg + "/full_backup/"
        ib_path = "/usr/local/Ncsis_Docker_Backup/backup/" + id + "_" + conimg + "/incremental_backup/"
